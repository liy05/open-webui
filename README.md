...
**当前状态**: 系统已调整为默认使用手机号码登录，暂时屏蔽PC端扫码登录功能。

- **默认登录方式**: 手机号码登录
- **企业微信登录**: 已暂时隐藏（可通过代码修改恢复）
- **其他登录方式**: 邮箱登录、LDAP登录、OAuth登录仍正常可用

#### 恢复企业微信登录的方法

如需重新启用企业微信登录，请在 `src/routes/auth/+page.svelte` 文件中：

1. 将第49行的 `let mode = 'phone';` 修改为原始逻辑
2. 将第317行的 `if (false && wecomConfig?.enabled)` 修改为 `if (wecomConfig?.enabled)`
3. 取消第335-340行企业微信自动初始化的注释
...
**修改内容**: 移除了聊天页面输入框为空时显示的呼叫按钮，统一显示发送按钮

**修改原因**: 
- 用户反馈在输入框为空时显示呼叫按钮容易造成误解
- 统一按钮显示逻辑，提供更一致的用户体验
- 简化界面操作，减少用户困惑

**具体变更**:
- 修改文件: `src/lib/components/chat/MessageInput.svelte`
- 移除了当 `prompt === '' && files.length === 0` 时显示呼叫按钮的条件判断
- 现在无论输入框是否为空，都统一显示发送按钮
- 发送按钮在输入框为空时会显示为禁用状态，有内容时才会激活
...
**修改内容**: 在移动端隐藏聊天界面的会话高级设置按钮

**修改原因**: 
- 移动端屏幕空间有限，高级设置按钮不是必需功能
- 简化移动端界面，避免界面过于拥挤
- 提升移动端用户体验，专注于核心聊天功能

**具体变更**:
- 修改文件: `src/lib/components/chat/Navbar.svelte`
  - 在Controls按钮外添加 `{#if !$mobile}` 条件判断，隐藏导航栏中的按钮
- 修改文件: `src/lib/components/layout/Navbar/Menu.svelte`
  - 移除移动端下拉菜单中的Controls选项，完全隐藏移动端的对话高级设置入口
- 确保移动端无法通过任何界面元素访问对话高级设置功能
...
**修改内容**: 当+号菜单中没有可选内容时，隐藏+号按钮

**修改原因**: 
- 避免显示空的菜单给用户造成困惑
- 简化界面，只在有功能可用时才显示相关按钮
- 提供更加智能的界面体验

**具体变更**:
- 修改文件: `src/lib/components/chat/MessageInput.svelte`
  - 添加条件判断：只有当存在工具、Google Drive集成或OneDrive集成时才显示+号按钮
  - 条件：`($tools && $tools.length > 0) || $config?.features?.enable_google_drive_integration || $config?.features?.enable_onedrive_integration`
- 修改文件: `src/lib/components/channel/MessageInput.svelte`
  - 在移动端完全隐藏+号按钮（因为移动端InputMenu只有截图功能，而截图功能已移到外部按钮）
  - 条件：`!$mobile`
